...
export const useGitStatusStore = create<GitStatusState>()(
			...
			fetchGitStatus: async (
				...
				try {
					...
					const response = await fetch(
						`/api/git/status?path=${encodeURIComponent(apiPath)}&showVirtualPaths=false`,
					);
...
// Selectors
export const selectGitStatus = (state: GitStatusState) => state.gitStatus;
export const selectShowGitStatus = (state: GitStatusState) => state.showGitStatus;
export const selectCurrentPath = (state: GitStatusState) => state.currentPath;
export const selectShowIgnoredFiles = (state: GitStatusState) => state.showIgnoredFiles;

// Commonly used combinations
export const selectGitStatusInfo = (state: GitStatusState) => ({
	gitStatus: state.gitStatus,
	showGitStatus: state.showGitStatus,
	currentPath: state.currentPath,
});

export const selectGitStatusUtils = (state: GitStatusState) => ({
...
export const gitStatusActions = {
	...
	setCurrentPath: (path: string) => useGitStatusStore.getState().setCurrentPath(path),
	fetchGitStatus: (directoryPath?: string) => useGitStatusStore.getState().fetchGitStatus(directoryPath),
	...
	getFileStatus: (filePath: string) => useGitStatusStore.getState().getFileStatus(filePath),
	isIgnored: (filePath: string) => useGitStatusStore.getState().isIgnored(filePath),
	get showGitStatus() {
