...
export async function GET(request: Request) {
  ...
  if (filePath.startsWith("~")) {
    ...
    filePath = path.isAbsolute(filePath) ? filePath : path.join(projectRoot, filePath);
  }

  // Ensure the path is within allowed base directory
  const projectRoot = process.cwd();
  if (!filePath.startsWith(BASE_PATH) && !filePath.startsWith(projectRoot)) {
    return NextResponse.json({ error: "Access denied: outside allowed directory" }, { status: 403 });
  }
