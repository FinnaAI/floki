...
console.log("[GitWorker] Initializing git status worker version:", WORKER_VERSION);
...
async function fetchGitStatus() {
    ...
    try {
        ...
        const response = await fetch(`/api/git/status?path=${encodeURIComponent(currentPath)}`);
        if (!response.ok) {
...
function startPolling() {
    ...
    stopPolling();
    console.log(`[GitWorker] Starting polling for path: ${currentPath} (interval: ${pollInterval}ms)`);
    // Initial fetch
    void fetchGitStatus();
    // Start polling
    ...
    switch (data.type) {
        case "setPath":
            if (data.path !== undefined) {
                ...
                console.log(`[GitWorker] Path ${oldPath ? `changed from ${oldPath}` : "set"} to: ${currentPath}`);
