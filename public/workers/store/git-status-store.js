...
const initWorker = (get) => {
    ...
    try {
        ...
        const workerUrl = new URL("/workers/workers/git-status.worker.js", window.location.origin);
...
export const useGitStatusStore = create((set, get) => {
    ...
    if (typeof window !== "undefined") {
        ...
        if (worker) {
            ...
            worker.onerror = (error) => {
                console.error("[GitStore] Worker error:", error.message || "Unknown worker error");
                set({
    ...
    return {
        ...
        getFileStatus: (filePath) => {
            ...
            for (const pathToCheck of pathsToCheck) {
                if (gitStatus.modified.includes(pathToCheck)) {
                    console.log(`[GitStatus] Found path ${pathToCheck} in modified files`);
                    return "modified";
                }
            }
            ...
            for (const pathToCheck of pathsToCheck) {
                if (gitStatus.untracked.includes(pathToCheck)) {
                    console.log(`[GitStatus] Found path ${pathToCheck} in untracked files`);
                    return "untracked";
                }
            }
        ...
        fetchGitStatus: async (directoryPath) => {
